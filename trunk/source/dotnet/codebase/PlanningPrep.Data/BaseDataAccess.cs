...
using PlanningPrep.Core.Logging;
...
namespace PlanningPrep.Data
{
        ...
        where TEntity : BaseEntity
    {
        ...
        {
            Database = DatabaseFactory.CreateDatabase("PlanningPrep.DbConnection");
        }
        ...
        protected internal object SaveInternal(string storedProc, IEnumerable<DbParameter> parameters /*, DbTransaction transaction*/)
        {
            ...
            using (DbCommand command = Database.GetStoredProcCommand(storedProc))
            {
                ...
                object returnValue;
                //returnValue = transaction == null ? Database.ExecuteNonQuery(command) : Database.ExecuteNonQuery(command, transaction);
        ...
        internal void SetParameters(DbCommand command, IEnumerable<DbParameter> parameters)
        {
            ...
            {
                if(typeof(IList<>).IsAssignableFrom(parameter.Value.GetType()) || typeof(IEnumerable<>).IsAssignableFrom(parameter.Value.GetType()))
                {
                    continue;
                }
        ...
        protected void HandleDataAccessException(Exception exception, string exceptionSource)
        {
            ...
            PlanningPrepLogger.HandleException(excToUse);
