...
namespace PlanningPrep.Data
{
    public sealed class DAOFactory
    {
        ...
        public static IDataAccess<T> Get<T>(IPrincipal currentUser) where T : BaseEntity
		{
            ...
            if (typeName.ToUpper() == "PLANNINGPREP.DATA.USERS.USERDAO")
            {
                assmbly = "PlanningPrep.Data";
                typeName = "PlanningPrep.Data.Users.UserDAO<>";
            }
            else if (typeName.ToUpper() == "PLANNINGPREP.DATA.ROLES.ROLESDAO")
            {
                assmbly = "PlanningPrep.Data";
                typeName = string.Format("PlanningPrep.Data.Roles.RoleDAO`1[{0}.{1}]", typeof(T).Namespace, typeof(T).Name);
            }
        ...
        static void LoadDAOs()
		{
            ...
            {              
				...
				try 
                {
					using (Stream Reader = Assembly.GetExecutingAssembly().GetManifestResourceStream("PlanningPrep.Data.DataAccessObjects.xml")) 
                    {
						daoDoc.Load(Reader);
					}
